...
def pregunta_01():
    ...
    for col in df.columns:
        ...
        plt.text(first_year - .2, df[col][first_year], col + " " + str(df[col][first_year]) + "%",
                 ha='right', va='center', color = colors[col])

        last_year = df.index[-1]
        plt.scatter(x=last_year, y=df[col][last_year], color=colors[col])

        plt.text(last_year - .2, df[col][last_year], col + " " + str(df[col][last_year]) + "%",
                 ha='right', va='center', color = colors[col])
